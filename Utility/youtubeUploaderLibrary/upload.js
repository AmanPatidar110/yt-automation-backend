...
async function uploadVideo(videoJSON, email) {
    ...
    if (
        await page
            .waitForXPath(createBtnXPath, { timeout: 5000 })
            .catch(() => null)
    ) {
    ...
    if (videoJSON.onProgress) {
        ...
        progressChecker = setInterval(async () => {
            ...
            const newProgress = curProgress
                ? parseInt(curProgress.slice(0, -1))
                : 0;
    ...
    if (!videoJSON.skipProcessingWait) {
        await page.waitForXPath(
            '//*[contains(text(),"Video upload complete")]',
            {
                hidden: true,
                timeout: 0,
            }
    ...
    if (playlistName) {
        ...
        for (let i = 0; i < 2; i++) {
            try {
                ...
                const playlistNameSelector = await page.$x(
                    playlistToSelectXPath
                );
                await page.evaluate(
                    (el) => el?.click(),
                    playlistNameSelector[0]
                );
