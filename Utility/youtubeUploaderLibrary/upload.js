...
const updateVideoInfo = async (videoJSON, messageTransport) => {
  ...
  const edit = await page.$x(editXpath)
  await edit[0].click()
    ...
    const [thumbChooser] = await Promise.all([
      page.waitForFileChooser({ timeout: 500 }).catch(async () => {
        ...
        await page.waitForXPath(
          "//*[normalize-space(text())='Save']/parent::*[@disabled]"
        )
        await sleep(500)
    ...
    try {
      switch (publish) {
        ...
        case 'public&premiere':
          await page.click(
            '#privacy-radios > tp-yt-paper-radio-button:nth-child(15)'
          )
